...
export class RouterOutlet extends HTMLElement implements IRouterOutlet {
	...
	public clearRoute (route: Route): void {
		...
		if (this.current != null && this.current.route.path.path === route.path.path) {
			if (this.current.instance.parentNode != null) this.current.instance.parentNode.removeChild(this.current.instance);
			this.current = null;
		}

		if (this.previous != null && this.previous.route.path.path === route.path.path) {
			if (this.previous.instance.parentNode != null) this.previous.instance.parentNode.removeChild(this.previous.instance);
			this.previous = null;
		}
