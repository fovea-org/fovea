import {ICSSTokenizerOptions} from "./i-css-tokenizer-options";
import {ICSSTokenizerResult} from "./i-css-tokenizer-result";
import {CSSToken, CSSTokenKind} from "./css-token";
 ...
 * @param {ICSSTokenizerInput} input
 * @param {Partial<ICSSTokenizerOptions>} options
 * @returns {ICSSTokenizerResult}
 */
export function cssTokenize (input: ICSSTokenizerInput, options: Partial<ICSSTokenizerOptions> = {}): ICSSTokenizerResult {
	...
	let prev: string;
	let n: number;
	let currentToken: CSSToken;

	const length = css.length;
	let offset = -1;
	let line = 1;
	let pos = 0;
	const buffer: CSSToken[] = [];
	const returned: CSSToken[] = [];

	/**
	 * Throws an error for an unclosed token
	 * @param {CSSTokenKind} what
	 */
	function unclosed (what: CSSTokenKind): void {
	 ...
	 */
	function back (token: CSSToken): void {
		returned.push(token);
	}
